...
class IrUiMenu(models.Model):
    ...
    def filter_mp_seller_menus(self,menu_ids):
        menu_objs = self.browse(menu_ids).exists()
        mp_menu_list = [self.env.ref('odoo_marketplace.wk_seller_website_menu').id,self.env.ref('odoo_marketplace.wk_seller_dashboard').id]
        for menu in menu_objs:
            if menu.parent_id.id in mp_menu_list:
                mp_menu_list.append(menu.id)
        return mp_menu_list
    ...
    def hide_mp_menus_to_user(self, menu_data):
        ...
        officer_group = self.env.ref(
            'odoo_marketplace.marketplace_officer_group').id
        groups_ids = self.env.user.sudo().groups_id.ids
        if officer_group in groups_ids:
            try:
                menu_ids.extend((
                    self.env.ref('odoo_marketplace.wk_seller_payment_request').id,
                ))
    ...
    def update_mp_menus(self, res):
        ...
        if self.env.user.has_group('odoo_marketplace.marketplace_draft_seller_group') and not self.env.user.has_group('odoo_marketplace.marketplace_officer_group'):
            seller_menu_id = self.env['ir.model.data'].check_object_reference(
                'odoo_marketplace', 'wk_seller_dashboard_menu1_sub_menu1')[1]
            seller_shops_menu_id = self.env['ir.model.data'].check_object_reference(
    ...
    def _visible_menu_ids(self, debug=False):
        ...
        if self.env.user.has_group('odoo_marketplace.marketplace_draft_seller_group') and not self.env.user.has_group('odoo_marketplace.marketplace_officer_group'):
            mp_menus = self.filter_mp_seller_menus(res)
            return set(mp_menus)
        return res
