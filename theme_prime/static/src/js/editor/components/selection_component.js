/** @odoo-module alias=theme_prime.selection_component */
import concurrency from 'web.concurrency';
import { registry } from '@web/core/registry';
import { qweb } from 'web.core';
import AbstractComponent from '@theme_prime/js/editor/components/abstract_component';
import { MarkupRecords } from 'theme_prime.mixins';

let SelectionComponent = AbstractComponent.extend(MarkupRecords, {
    template: 'theme_prime.selection_component',

    xmlDependencies: AbstractComponent.prototype.xmlDependencies.concat(['/theme_prime/static/src/xml/editor/components/selection_component.xml']),
    events: {
    ...
    _renderDropdown: function (records) {
        ...
        if (dropdownTemplate) {
            var $menu = $(qweb.render('theme_prime.tp_autocomplete_dropdown_item', {
    ...
    _renderSuggestions: async function () {
        var $suggestions = $(qweb.render('theme_prime.suggestions', {suggestions: this.suggestions, widget:this}));
...
registry.category("theme_prime_components").add("SelectionComponent", SelectionComponent);
